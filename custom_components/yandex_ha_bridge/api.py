...
class YandexApi:
    """Minimal async client for Yandex Smart Home API."""
    ...
    async def _request(self, method: str, path: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        def request() -> dict[str, Any]:
            ...
            req = urllib.request.Request(API + path, data=data, headers={"Authorization": f"Bearer {self.token}", "Accept": "application/json", "Content-Type": "application/json", "User-Agent": f"Yandex-HABridge/{VERSION}"}, method=method)
            try:
                ...
                if request_id: message += f" (request_id={request_id})"

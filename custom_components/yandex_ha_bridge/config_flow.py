"""Config flow for Yandex HA Bridge."""
...
class YandexConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle Yandex HA Bridge configuration."""
    ...
    async def async_step_devices(self, user_input=None):
        if user_input is not None:
            ...
            return self.async_create_entry(
                title=NAME,
                data={CONF_TOKEN: self._token, CONF_DEVICE_IDS: ids},
            )

        options = [
            selector.SelectOptionDict(
                value=str(d.get("id")),
                label=f"{d.get('name', 'Без имени')} — {(d.get('device_info') or {}).get('model', '—')}",
            )
            for d in self._devices
        ]
        schema = vol.Schema({
            vol.Optional(CONF_DEVICE_IDS, default=[]): selector.SelectSelector(
...
class YandexOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        schema = vol.Schema({
            vol.Optional(CONF_DEVICE_IDS, default=selected): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=options,
                    multiple=True,
                    mode=selector.SelectSelectorMode.LIST,

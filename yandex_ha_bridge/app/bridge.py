...
def render_page(devices, selected):
    rows = []
    for d in devices:
        device_id = str(d.get("id", "")); info = d.get("device_info", {}) or {}
        name = html.escape(str(d.get("name") or "Без имени")); model = html.escape(str(info.get("model") or "—")); dtype = html.escape(str(d.get("type") or "—"))
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path != "/": self.send_html(404, "<h1>404</h1>"); return
        try: self.send_html(200, render_page(fetch_devices(self.server.token), load_selected()))
        except urllib.error.HTTPError as e: self.send_html(502, f"<h1>Ошибка API: HTTP {e.code}</h1><p>Проверьте OAuth-токен и права iot:view.</p>")
        except Exception: LOG.exception("Ошибка Web UI"); self.send_html(500, "<h1>Ошибка</h1><p>Не удалось получить список устройств.</p>")
    def do_POST(self):
        if self.path != "/save": self.send_html(404, "<h1>404</h1>"); return
        length = int(self.headers.get("Content-Length", "0")); values = urllib.parse.parse_qs(self.rfile.read(length).decode("utf-8")).get("device_id", [])
        try:
            valid_ids = {str(d.get("id")) for d in fetch_devices(self.server.token)}; save_selected([x for x in values if x in valid_ids]); self.send_response(303); self.send_header("Location", "/"); self.end_headers()
        except Exception: LOG.exception("Не удалось сохранить выбор устройств"); self.send_html(500, "<h1>Ошибка сохранения</h1>")


def start_web(token):
    server = ThreadingHTTPServer(("0.0.0.0", WEB_PORT), WebHandler); server.token = token
    LOG.info("Веб-интерфейс диагностики запущен на порту %s", WEB_PORT); threading.Thread(target=server.serve_forever, daemon=True).start()


def main():
    options = load_options(); token = (options.get("yandex_oauth_token") or "").strip(); interval = max(10, int(options.get("poll_interval", 30))); level = getattr(logging, str(options.get("log_level", "info")).upper(), logging.INFO)
    logging.basicConfig(level=level, format="%(asctime)s %(levelname)s %(name)s: %(message)s"); LOG.info("Yandex HA Bridge v%s запускается", VERSION)
    if not token:
        LOG.error("OAuth-токен Яндекса не настроен. Добавьте его в разделе «Конфигурация» приложения.")
        while True: time.sleep(300)
    start_web(token); LOG.info("Основная работа с устройствами выполняется через интеграцию Yandex HA Bridge в Home Assistant")
    while True:
        try:
            data = api_get("/v1.0/user/info", token); LOG.info("Соединение с API Яндекс Умного дома установлено"); selected = load_selected()
            for device in [d for d in data.get("devices", []) if str(d.get("id")) in selected]:
                state = api_get("/v1.0/devices/" + str(device["id"]), token); LOG.debug("Состояние %s: %s", device.get("name"), json.dumps(state, ensure_ascii=False))
        except urllib.error.HTTPError as e: LOG.error("Ошибка авторизации Яндекс API (HTTP %s). Проверьте OAuth-токен и права iot:view.", e.code)
        except urllib.error.URLError as e: LOG.error("Не удалось подключиться к API Яндекса: %s", e.reason)
        except Exception as e: LOG.exception("Ошибка цикла моста: %s", e)
        time.sleep(interval)
